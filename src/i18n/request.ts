import { getRequestConfig } from 'next-intl/server';
import { routing } from './routing';

export default getRequestConfig(async ({ locale }) => {
  let safeLocale = (locale ?? routing.defaultLocale) as (typeof routing.locales)[number];

  // ✅ Normalize English variants to 'en'
  if (safeLocale.toLowerCase().startsWith('en')) {
    safeLocale = 'en';
  }

  if (!routing.locales.includes(safeLocale)) {
    throw new Error(`Unsupported locale: ${safeLocale}`);
  }

  const messages = {

    // Existing namespaces
    adoptionApplicationForm: (await import(`../../messages/${safeLocale}/adoptionApplicationForm.json`)).default,
    boardingAndDaycareDashboard: (await import(`../../messages/${safeLocale}/boardingAndDaycareDashboard.json`)).default,
    boardingAndDaycareAddDogClientDogs: (await import(`../../messages/en/boardingAndDaycareAddDogClientDogs.json`)).default,
    boardingAndDaycareAddDogClientList: (await import(`../../messages/${safeLocale}/boardingAndDaycareAddDogClientList.json`)).default,
    boardingAndDaycareBusinessSettings: (await import(`../../messages/${safeLocale}/boardingAndDaycareBusinessSettings.json`)).default,
    boardingAndDaycareBusinessSettingsDiscounts: (await import(`../../messages/${safeLocale}/boardingAndDaycareBusinessSettingsDiscounts.json`)).default,
    boardingAndDaycareBusinessSettingsMemberships: (await import(`../../messages/${safeLocale}/boardingAndDaycareBusinessSettingsMemberships.json`)).default,
    boardingAndDaycareBookingMembershipSelector: (await import(`../../messages/${safeLocale}/boardingAndDaycareBookingMembershipSelector.json`)).default,
    boardingAndDaycarePendingRequests: (await import(`../../messages/${safeLocale}/boardingAndDaycarePendingRequests.json`)).default,
    boardingAndDaycareClientManagement: (await import(`../../messages/${safeLocale}/boardingAndDaycareClientManagement.json`)).default,
    boardingAndDaycareEmployeeManagementDashboard: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementDashboard.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeScheduling: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeScheduling.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingConflictDetection: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingConflictDetection.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingCoverageView: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingCoverageView.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingCreateShift: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingCreateShift.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingEmployeeAvailability: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingEmployeeAvailability.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingScheduleCalendar: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingScheduleCalendar.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingShiftTemplateEditor: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingShiftTemplateEditor.json`)).default,
    boardingAndDaycareEmployeeManagementEmployeeSchedulingShiftTemplates: (await import(`../../messages/${safeLocale}/boardingAndDaycareEmployeeManagementEmployeeSchedulingShiftTemplates.json`)).default,
    boardingAndDaycareFinancialManagement: (await import(`../../messages/${safeLocale}/boardingAndDaycareFinancialManagement.json`)).default,
    boardingAndDaycareIndividualClient: (await import(`../../messages/${safeLocale}/boardingAndDaycareIndividualClient.json`)).default,
    boardingAndDaycareIndividualClientNotes: (await import(`../../messages/${safeLocale}/boardinganddaycareindividualclientnotes.json`)).default,
    boardingAndDaycareInvoiceLibrary: (await import(`../../messages/${safeLocale}/boardingAndDaycareInvoiceLibrary.json`)).default,
    boardingAndDaycareDogsOnProperty: (await import(`../../messages/${safeLocale}/boardingAndDaycareDogsOnProperty.json`)).default,
    boardingAndDaycareManualInvoices: (await import(`../../messages/${safeLocale}/boardingAndDaycareManualInvoices.json`)).default,
    boardingAndDaycarePayAtCheckoutItems: (await import(`../../messages/${safeLocale}/boardingAndDaycarePayAtCheckoutItems.json`)).default,
    boardingAndDaycarePayAtCheckoutBoardingItems: (await import(`../../messages/${safeLocale}/boardingAndDaycarePayAtCheckoutBoardingItems.json`)).default,
    boardingAndDaycarePayAtCheckoutDaycareItems: (await import(`../../messages/${safeLocale}/boardingAndDaycarePayAtCheckoutDaycareItems.json`)).default,
    boardingAndDaycarePayAtCheckoutGroomingItems: (await import(`../../messages/${safeLocale}/boardingAndDaycarePayAtCheckoutGroomingItems.json`)).default,
    boardingAndDaycareManageIndividualEmployee: (await import(`../../messages/${safeLocale}/boardingAndDaycareManageIndividualEmployee.json`)).default,
    boardingAndDaycareUploadVaccineFile: (await import(`../../messages/${safeLocale}/boardingAndDaycareUploadVaccineFile.json`)).default,
    boardingAndDaycareUploadFecalFile: (await import(`../../messages/${safeLocale}/boardingAndDaycareUploadFecalFile.json`)).default,
    boardingAndDaycareMessageClient: (await import(`../../messages/${safeLocale}/boardingAndDaycareMessageClient.json`)).default,
    breederBusinessSettings: (await import(`../../messages/${safeLocale}/breederBusinessSettings.json`)).default,
    breederClientManagement: (await import(`../../messages/${safeLocale}/breederClientManagement.json`)).default,
    breederDashboard: (await import(`../../messages/${safeLocale}/breederDashboard.json`)).default,
    breederMyDogs: (await import(`../../messages/${safeLocale}/breederMyDogs.json`)).default,
    breederPendingRequests: (await import(`../../messages/${safeLocale}/breederPendingRequests.json`)).default,
    breederApplicationsAndContracts: (await import(`../../messages/${safeLocale}/breederApplicationsAndContracts.json`)).default,
    breederSubmittedApplications: (await import(`../../messages/${safeLocale}/breederSubmittedApplications.json`)).default,
    breederSubmittedContracts: (await import(`../../messages/${safeLocale}/breederSubmittedContracts.json`)).default,
    businessSignUp: (await import(`../../messages/${safeLocale}/businessSignUp.json`)).default,
    groomingDashboard: (await import(`../../messages/${safeLocale}/groomingDashboard.json`)).default,
    individualSignUp: (await import(`../../messages/${safeLocale}/individualSignUp.json`)).default,
    individualDashboard: (await import(`../../messages/${safeLocale}/individualDashboard.json`)).default,
    individualEditProfile: (await import(`../../messages/${safeLocale}/individualEditProfile.json`)).default,
    individualSearchBusinesses: (await import(`../../messages/${safeLocale}/individualSearchBusinesses.json`)).default,
    individualSendClientRequest: (await import(`../../messages/${safeLocale}/individualSendClientRequest.json`)).default,
    individualReminders: (await import(`../../messages/${safeLocale}/individualReminders.json`)).default,
    individualMyPets: (await import(`../../messages/${safeLocale}/individualMyPets.json`)).default,
    individualAddEditPet: (await import(`../../messages/${safeLocale}/individualAddEditPet.json`)).default,
    individualNotifications: (await import(`../../messages/${safeLocale}/individualNotifications.json`)).default,
    individualUpcomingAppointments: (await import(`../../messages/${safeLocale}/individualUpcomingAppointments.json`)).default,
    individualBookServices: (await import(`../../messages/${safeLocale}/individualBookServices.json`)).default,
    individualSelectService: (await import(`../../messages/${safeLocale}/individualSelectService.json`)).default,
    individualBookDaycare: (await import(`../../messages/${safeLocale}/individualBookDaycare.json`)).default,
    individualBookBoarding: (await import(`../../messages/${safeLocale}/individualBookBoarding.json`)).default,
    individualBookAssessment: (await import(`../../messages/${safeLocale}/individualBookAssessment.json`)).default,
    individualEmployeeDogsOnProperty: (await import(`../../messages/${safeLocale}/individualEmployeeDogsOnProperty.json`)).default,
    individualEmployeePortalCompanyPolicies: (await import(`../../messages/${safeLocale}/individualEmployeePortalCompanyPolicies.json`)).default,
    individualEmployeePortalDashboard: (await import(`../../messages/${safeLocale}/individualEmployeePortalDashboard.json`)).default,
    individualEmployeePortalDocumentsToSign: (await import(`../../messages/${safeLocale}/individualEmployeePortalDocumentsToSign.json`)).default,
    individualEmployeePortalMyAvailability: (await import(`../../messages/${safeLocale}/individualEmployeePortalMyAvailability.json`)).default,
    individualEmployeePortalSignedDocuments: (await import(`../../messages/${safeLocale}/individualEmployeePortalSignedDocuments.json`)).default,
    individualEmployeePortalTimeOffRequests: (await import(`../../messages/${safeLocale}/individualEmployeePortalTimeOffRequests.json`)).default,
    individualEmployeePortalTraining: (await import(`../../messages/${safeLocale}/individualEmployeePortalTraining.json`)).default,
    individualEmployeeUpcomingReservations: (await import(`../../messages/${safeLocale}/individualEmployeeUpcomingReservations.json`)).default,
    individualMessages: (await import(`../../messages/${safeLocale}/individualMessages.json`)).default,
    individualMessageBusiness: (await import(`../../messages/${safeLocale}/individualMessageBusiness.json`)).default,
    waiverAgreement: (await import(`../../messages/${safeLocale}/waiverAgreement.json`)).default,
    walkerSitterBusinessSettings: (await import(`../../messages/${safeLocale}/walkerSitterBusinessSettings.json`)).default,
    walkerSitterClientManagement: (await import(`../../messages/${safeLocale}/walkerSitterClientManagement.json`)).default,
    walkerSitterDashboard: (await import(`../../messages/${safeLocale}/walkerSitterDashboard.json`)).default,
    walkerSitterPendingRequests: (await import(`../../messages/${safeLocale}/walkerSitterPendingRequests.json`)).default,
    walkerSitterTodayVisits: (await import(`../../messages/${safeLocale}/walkerSitterTodaysVisits.json`)).default
  } as const;

  return {
    locale: safeLocale,
    messages
  };
});
