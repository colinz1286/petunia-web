...
export default function BoardingAndDaycareUploadFecalFilePage() {
    const t = useTranslations('boardingAndDaycareUploadFecalFile');
    ...
    useEffect(() => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            try {
                ...
                console.error('❌ Error loading fecal files:', err);
    ...
    const loadExistingFiles = async () => {
        const folderRef = ref(storage, `fecalTests/${userId}`);
        ...
        try {
            ...
            for (let i = 1; i <= 2; i++) {
                for (const ext of cleanupExtensions) {
                    const path = `fecalTests/${userId}/${petId}_${i}.${ext}`;
            ...
            for (let idx = 0; idx < toUpload.length; idx++) {
                ...
                const fullPath = `fecalTests/${userId}/${petId}_${idx + 1}.${ext}`;
            ...
            console.error('❌ Upload fecal error:', err);
    ...
    const handleDelete = async (file: RemoteFile) => {
        try {
            ...
            console.error('❌ Delete fecal file error:', err);
