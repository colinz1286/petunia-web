...
export default function GroomingDashboardPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
            ...
            // ✅ Fetch business info and then attach unread listener with the resolved id
    ...
    const listenForUnreadMessages = (bizId: string) => {
        const q = query(
            collectionGroup(db, 'threadMessages'),
            where('receiverId', '==', bizId),
            where('read', '==', false)
        );
        ...
        return onSnapshot(q, (snapshot) => {
            ...
            console.log(`📩 Unread messages for ${bizId}: ${unreadCount}`);
        });
    };
        ...
        <div className="min-h-screen bg-[color:var(--color-background)] text-[color:var(--color-foreground)] px-4 py-6">
            <div className="w-full max-w-md mx-auto px-2 sm:px-4">
                ...
                <div className="space-y-3">
                    {/* Label change only: Dogs In Salon */}
                    <DashboardLink href="/groomingdogsonproperty" label="Dogs In Salon" />

                    {/* Label change only: Upcoming Appointments */}
                    <DashboardLink href="/groomingupcomingappointments" label="Upcoming Appointments" />

                    <DashboardLink href="/grooming-pendingrequests" label={t('pending_requests')} />
                    <DashboardLink href="/groomingnotificationsview" label={t('notifications')} />

                    {/* 📨 Messages button with red dot */}
                    <DotLink href="/groomingmessages" label="Messages" showDot={hasUnreadMessages} />

                    <DashboardLink href="/grooming-clientmanagement" label={t('client_management')} />

                    {enableEmployeeManagement && (
                        <>
                            <DashboardLink href="/grooming-manageemployees" label={t('manage_employees')} />
                            <DashboardLink href="/grooming-scheduleemployees" label={t('schedule_employees')} />
                        </>
                    )}

                    {enableStatePaperwork && (
                        <DashboardLink href="/grooming-statepaperworklog" label={t('state_paperwork_log')} />
                    )}

                    <DashboardLink href="/grooming-businesssettings" label={t('business_settings')} />
