  ...
  const checkIn = useCallback(
    async (g: GroupedReservation) => {
      ...
      if (data?.petStatuses) {
        ...
        const allDone = (Object.values(data.petStatuses) as string[]).every((v) =>
          v === 'checkedIn' || v === 'noShow',
        );
  ...
  const markNoShow = useCallback(
    async (g: GroupedReservation) => {
      ...
      if (data?.petStatuses) {
        ...
        const allDone = (Object.values(data.petStatuses) as string[]).every(
    ...
    async (g: GroupedReservation) => {
      ...
      await Promise.all(
        Object.keys(g.petStatuses).map(async (petId) => {
          ...
          await rtdbRemove(
            rtdbRef(rtdb, `upcomingReservations/${businessId}/${g.realtimeKey}-${petId}`),
          );
        }),
      );
  ...
  return (
    <div className="min-h-screen bg-[color:var(--color-background)] text-[color:var(--color-foreground)]">
      <main className="max-w-3xl mx-auto px-4 py-6">
        ...
        {isLoading && <p className="text-center text-sm text-gray-600">{t('loading')}</p>}
        {!isLoading && errorMsg && (
                  ...
                  {expandedDates.has(dateKey) && (
                    <div className="px-3 pb-3">
                      {(groupedByDate[dateKey] || []).map((g) => {
                        const lower = (g.dogNames[0] || '').toLowerCase();
                        const hasMeds =
                          (g.medications[lower] || '').trim().toLowerCase() === 'yes';
                        const intact =
                          (g.spayedNeutered[lower] || '').trim().toLowerCase() === 'no';
                              ...
                              {g.type === 'Boarding' ? (
                                <>
                                  <div>
                                    ...
                                    {g.boardingCheckInDate
                                      ? formatISODateLong(g.boardingCheckInDate)
                                      : '—'}
