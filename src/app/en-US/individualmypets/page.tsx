...
export default function IndividualMyPetsPage() {
  ...
  useEffect(() => {
    let stopPetsListener: (() => void) | undefined;
    ...
    const stopAuth = onAuthStateChanged(auth, (user) => {
      ...
      // Start a real-time listener so UI == Firestore at all times.
      const petsRef = collection(db, 'users', uid, 'pets');
      const qPets = query(petsRef, orderBy('createdAt', 'asc'));

      stopPetsListener?.();
      stopPetsListener = onSnapshot(
        qPets,
        (snapshot) => {
          const list = snapshot.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<Pet, 'id'>) })) as Pet[];

          // De-duplicate defensively by id (protects against any accidental double-writes or double-mounts)
          const unique = Array.from(new Map(list.map((p) => [p.id, p])).values());

          setPets(unique);
          setLoading(false);

          // If the expanded item was deleted elsewhere, collapse it.
          if (expandedId && !unique.some((p) => p.id === expandedId)) {
            setExpandedId(null);
          }
        },
        (err) => {
    ...
    // Only re-run when locale changes (router object is stable but not needed here)
  }, [locale, router, expandedId]);

  const handleDelete = async (pet: Pet) => {
    try {
      await deleteDoc(doc(db, 'users', userId, 'pets', pet.id));
      // Best-effort cleanup of a common vaccine file name; UI still mirrors Firestore via listener.
  ...
  return (
    <main className="min-h-screen bg-[var(--BrandBackground)] px-4 py-6 text-[color:var(--color-foreground)]">
      <div className="w-full max-w-md mx-auto px-2 sm:px-4">
                ...
                >
                  ...
                  {isOpen && (
                    <div className="pt-3 text-sm text-gray-700 space-y-2">
                      <p>
                        {t('type_label')}: {pet.petType}
                      </p>
                      <p>
                        {t('breed_label')}: {pet.breed}
                      </p>
                      <p>
                        {t('age_label')}:{' '}
                        {pet.ageValue && pet.ageUnit ? `${pet.ageValue} ${pet.ageUnit}` : ''}
                      </p>
                      <p>
                        {t('weight_label')}: {pet.weight}
                      </p>
                      <p>
                        {t('gender_label')}: {pet.gender}
                      </p>
                      <p>
                        {t('spayed_neutered_label')}: {pet.spayedNeutered}
                      </p>
                      <p>
                        {t('allergies_label')}: {pet.allergies}
                      </p>
                      <p>
                        {t('current_food_label')}: {pet.currentFood}
                      </p>
                      <p>
                        {t('feeding_label')}: {pet.feedingAmount} {t('cups_label')},{' '}
                        {pet.feedingFrequency}x/{t('day_label')}
                      </p>
                      <p>
                        {t('veterinarian_label')}: {pet.veterinarian}
                      </p>
                      <p>
                        {t('medications_label')}: {pet.medications}
                      </p>
                      {pet.medicationDetails && (
                        <p>
                          {t('medication_details_label')}: {pet.medicationDetails}
                        </p>
                      )}
                      {pet.boardingExperienceDetails && (
                        <p>
                          {t('boarding_experience_label')}: {pet.boardingExperienceDetails}
                        </p>
                      )}
                      {pet.fearDetails && (
                        <p>
                          {t('fears_label')}: {pet.fearDetails}
                        </p>
                      )}
                      ...
                      <div className="flex justify-between pt-4">
                        <button
                          ...
                          onClick={() =>
                            router.push(`/${locale}/individualaddeditpet?petId=${pet.id}`)
                          }
                        >
                          {t('edit_pet_button')}
                        </button>
        ...
        <div className="mt-10">
          <Link
            href={`/${locale}/individualaddeditpet`}
