...
const FORBIDDEN_FEEDING_CHARS = /[\/\.\$\[\]#]/; // eslint-disable-line @typescript-eslint/no-unused-vars
...
export default function AddEditPetPage() {
    ...
    const formatDateFromAny = useCallback((value: unknown): string => {
        if (!value) return '';
        if (typeof value === 'string') return value; // assume YYYY-MM-DD
    ...
    const loadPetData = useCallback(async (uid: string, id: string) => {
        ...
        // Load up to two existing vaccine files by index and by any supported extension (de-duped)
        const foundByIndex = new Map<number, RemoteVaccineFile>();
        for (const index of [1, 2] as const) {
            for (const ext of VACCINE_EXTS) {
                const path = `vaccineRecords/${uid}/${id}_${index}.${ext}`;
                try {
                    const url = await getDownloadURL(storageRef(storage, path));
                    if (!foundByIndex.has(index)) {
                        foundByIndex.set(index, { index, name: `${id}_${index}.${ext}`, url, ext });
                    }
                    break; // first ext wins for this index
                } catch {
                    // not found; keep trying other extensions
                }
            }
        }
    ...
    useEffect(() => {
        ...
        return () => unsub();
    }, [router, locale, petId, loadPetData]);
    ...
    const handleNewVaccineFiles = (files: FileList | null): void => {
        ...
        const ALLOWED_EXTS = new Set<string>(['pdf', 'jpg', 'jpeg', 'png', 'heic']);

        // De-duplicate by filename (case-insensitive) so the same file isn't shown twice
        const existingNames = new Set<string>([
            ...newVaccineFiles.map(f => f.name.toLowerCase()),
            ...remoteVaccineFiles.map(r => r.name.toLowerCase()),
        ]);

        const next: File[] = [...newVaccineFiles];

        for (const f of Array.from(files)) {
            if (next.length >= 2) break; // mirror iOS limit

            const nameLower = f.name.toLowerCase();
            if (existingNames.has(nameLower)) continue; // skip duplicates in UI

            const ext = (extOf(f.name) ?? '').toLowerCase();
            if (!ALLOWED_EXTS.has(ext)) {
                alert(
                    t('vaccine_ext_invalid', {
                        defaultValue: 'Unsupported file type. Allowed: pdf, jpg, jpeg, png, heic.',
                    })
                );
                continue;
            }
            ...
            next.push(f);
            existingNames.add(nameLower); // prevent dupes within the same selection
        }

        setNewVaccineFiles(next.slice(0, 2));
    };
        ...
        try {
            ...
            (Object.keys(vaccinationRecords) as VaccineKey[]).forEach((k) => {
                ...
                vaxPayload[k] = {
                    ...
                    date: item.isVaccinated && item.expirationDate ? toTimestampOrNull(item.expirationDate) : null,
                };
            });
            ...
            <div className="w-full max-w-xl mx-auto px-2 sm:px-4">
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" className={INPUT} value={petName} onChange={(e) => setPetName(e.target.value)} />
                    </div>
                    ...
                    <div>
                        ...
                        <select value={spayedNeutered} onChange={(e) => setSpayedNeutered(e.target.value as 'Yes' | 'No')} className={SELECT}>
                    ...
                    <div>
                        ...
                        <input type="date" className={INPUT} value={dateOfBirth} onChange={(e) => onDOBChange(e.target.value)} />
                    </div>
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <select value={ageUnit} onChange={(e) => setAgeUnit(e.target.value as 'Years' | 'Months')} className={SELECT}>
                    ...
                    <div>
                        ...
                        <input type="date" className={INPUT} value={lastAnnualVetVisit} onChange={(e) => setLastAnnualVetVisit(e.target.value)} />
                    </div>

                    {/* Weight */}
                    <div>
                        <label className={LABEL}>{t('weight', { defaultValue: 'Weight (lbs)' })}</label>
                        <input type="text" className={INPUT} value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="e.g., 45" />
                    </div>

                    {/* Allergies */}
                    <div>
                        <label className={LABEL}>{t('allergies', { defaultValue: 'Allergies' })}</label>
                        <input type="text" className={INPUT} value={allergies} onChange={(e) => setAllergies(e.target.value)} placeholder={t('none', { defaultValue: 'None' })} />
                    </div>

                    {/* Current Food */}
                    <div>
                        <label className={LABEL}>{t('current_food', { defaultValue: 'Current Food' })}</label>
                        <input type="text" className={INPUT} value={currentFood} onChange={(e) => setCurrentFood(e.target.value)} placeholder="e.g., Acana" />
                    </div>

                    {/* Cups per Meal */}
                    <div>
                        <label className={LABEL}>{t('cups_per_meal', { defaultValue: 'Cups per Meal' })}</label>
                        <select value={cupsPerMeal} onChange={(e) => setCupsPerMeal(e.target.value)} className={SELECT}>
                    ...
                    {/* Hidden Feeding Frequency parity field */}
                    <input type="hidden" value={feedingFrequency} readOnly />
                    ...
                    <div>
                        ...
                        <label className={CHECK_LABEL}>
                            <input type="checkbox" className="accent-[#2c4a30]" checked={receivesBreakfast} onChange={(e) => setReceivesBreakfast(e.target.checked)} />
                        ...
                        {/* Lunch */}
                        <label className={CHECK_LABEL}>
                            <input type="checkbox" className="accent-[#2c4a30]" checked={receivesLunch} onChange={(e) => setReceivesLunch(e.target.checked)} />
                        ...
                        {/* Dinner */}
                        <label className={CHECK_LABEL}>
                            <input type="checkbox" className="accent-[#2c4a30]" checked={receivesDinner} onChange={(e) => setReceivesDinner(e.target.checked)} />
                    ...
                    <div>
                        ...
                        <input type="text" autoComplete="off" className={INPUT} value={veterinarian} onChange={(e) => setVeterinarian(e.target.value)} placeholder={t('veterinarian', { defaultValue: 'Veterinarian Name' })} />
                    </div>

                    {/* Vet Phone */}
                    <div>
                        <label className={LABEL}>{t('veterinarian_phone', { defaultValue: 'Veterinarian Phone' })}</label>
                        <input type="text" autoComplete="off" className={INPUT} value={veterinarianPhone} onChange={(e) => setVeterinarianPhone(e.target.value)} placeholder={t('veterinarian_phone', { defaultValue: 'Veterinarian Phone' })} />
                    </div>

                    {/* Medications */}
                    <div>
                        <label className={LABEL}>{t('medications_question', { defaultValue: 'Medications?' })}</label>
                        <select value={medications} onChange={(e) => setMedications(e.target.value as 'Yes' | 'No')} className={SELECT}>
                            <option value="No">{t('no', { defaultValue: 'No' })}</option>
                            <option value="Yes">{t('yes', { defaultValue: 'Yes' })}</option>
                        </select>
                        {medications === 'Yes' && (
                    ...
                    <div>
                        ...
                        <select value={boardingExperience} onChange={(e) => setBoardingExperience(e.target.value as 'Yes' | 'No')} className={SELECT}>
                            <option value="No">{t('no', { defaultValue: 'No' })}</option>
                            <option value="Yes">{t('yes', { defaultValue: 'Yes' })}</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select value={afraidOfAnything} onChange={(e) => setAfraidOfAnything(e.target.value as 'Yes' | 'No')} className={SELECT}>
                            <option value="No">{t('no', { defaultValue: 'No' })}</option>
                            <option value="Yes">{t('yes', { defaultValue: 'Yes' })}</option>
                        </select>
                    ...
                    <div>
                        ...
                        {/* Upload control */}
                        <div className="mb-2">
                            <label htmlFor="vaccine-uploads" className="underline cursor-pointer text-sm text-[#2c4a30] hover:text-[#1f3523]">
                        ...
                        {remoteVaccineFiles.length > 0 && (
                            <div className="space-y-2 mb-3">
                                {remoteVaccineFiles.map((rf) => (
                                    <div key={`${rf.index}-${rf.name}`} className="flex items-center justify-between bg-[#f7f3e9] border border-[#2c4a30] p-2 rounded text-sm">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <button type="button" onClick={() => removeRemoteFile(rf)} className="text-red-600 underline">
                        ...
                        {newVaccineFiles.length > 0 && (
                            <div className="space-y-2">
                                {newVaccineFiles.map((f, idx) => (
                                    <div key={idx} className="flex items-center justify-between bg-[#fbf9f3] border border-[#2c4a30] p-2 rounded text-sm">
                                        <span className="truncate">{f.name}</span>
                                        <div className="flex items-center gap-4">
                                            <button type="button" onClick={() => removePendingNewFile(idx)} className="text-red-600 underline">
                                                {t('remove_file', { defaultValue: 'Remove' })}
                                            </button>
                                        </div>
                                    </div>
                    ...
                    {/* Submit */}
                    <button type="submit" disabled={saving} className="w-full bg-[#2c4a30] text-white py-3 rounded hover:bg-[#1f3523] transition">
