...
export default function AddEditPetPage() {
  ...
  // ---------- Helpers ----------
  const formatDateFromAny = (value: unknown): string => {
    if (!value) return '';
    if (typeof value === 'string') return value; // assume YYYY-MM-DD
    if (typeof value === 'object' && value !== null && 'toDate' in value && typeof (value as any).toDate === 'function') {
      const iso = (value as any).toDate().toISOString();
  ...
  const loadPetData = async (uid: string, id: string) => {
    ...
    const getDateStr = (obj: any): string => {
      if (!obj) return '';
  ...
  const handleNewVaccineFiles = (files: FileList | null) => {
    if (!files || files.length === 0) return;
    const existing = [...newVaccineFiles];
    for (const f of Array.from(files)) {
      if (existing.length >= 2) break; // mirror iOS limit
      const ext = extOf(f.name);
      if (!VACCINE_EXTS.includes(ext as any)) {
        alert(t('vaccine_ext_invalid', { defaultValue: 'Unsupported file type. Allowed: pdf, jpg, jpeg, png, heic.' }));
        continue;
      }
      existing.push(f);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const vaxPayload: Record<string, any> = {};
      ...
      // ---- Vaccine Files (mirror iOS: support two slots; clean up per index before upload) ----
      // 1) For any new files selected, delete any existing variants for that index and upload the new file.
      const toUpload = newVaccineFiles.slice(0, 2); // assign in order to index 1, 2
      ...
      // 2) Remote files removed by the user are already deleted immediately in removeRemoteFile().
      //    For untouched remote files (and no new file takes their index), we leave them as-is.
