...
const pickBusinessId = (snap: QuerySnapshot<DocumentData>): string | null => {
    ...
    // Fallback: if user only has one business or the type is not set, just take the first.
...
function AddEditDogModal({
    ...
    const isValidForm = useMemo(() => {
        ...
        // If your DB may contain breeds outside the condensed list, allow any non-empty.
        ...
        try {
            ...
            if (mode === 'add') {
                ...
                const merged: BreedingDog = {
                    ...
                    bodyWeightLbs:
                        typeof weightVal === 'number' && Number.isFinite(weightVal) ? weightVal : existingDog?.bodyWeightLbs,
                    bodyWeightUpdatedAt:
                        typeof weightVal === 'number' && Number.isFinite(weightVal) ? new Date() : bodyWeightUpdatedAt,
                };
                onSaved(merged);
                onClose();
            }
